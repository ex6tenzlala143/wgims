...
<div style="display:grid;grid-template-columns:2fr 1fr;gap:24px">
    ...
    <div>
        ...
        {{-- Per-item sections --}}
        @php $completedCount = $deliverySubsidy->items->filter(fn($i) => $i->quantity - $i->qty_delivered <= 0)->count(); @endphp
        @if($completedCount > 0)
        <div style="margin-bottom:12px;display:flex;align-items:center;gap:10px">
            <span style="font-size:13px;color:var(--text-muted)">
                <i class="fas fa-check-circle" style="color:var(--success)"></i>
                {{ $completedCount }} item(s) fully delivered and hidden.
            </span>
            <button type="button" id="toggle-completed-btn" onclick="toggleCompleted()"
                    class="btn btn-sm btn-outline" style="font-size:12px;padding:3px 10px">
        ...
        <div class="card item-section-card {{ $isDone ? 'item-completed' : '' }}"
             style="margin-bottom:16px;{{ $isDone ? 'display:none' : '' }}"
             id="item-section-{{ $poiIdx }}">
            {{-- Item header --}}
            ...
            <div id="batches-{{ $poiIdx }}">
                @if($isDone)
                {{-- Completed item: submit zeros via hidden inputs so server validation passes --}}
                <input type="hidden" name="items[{{ $poiIdx }}_0][ds_item_id]"         value="{{ $poi->id }}">
                <input type="hidden" name="items[{{ $poiIdx }}_0][quantity_delivered]"  value="0">
                <input type="hidden" name="items[{{ $poiIdx }}_0][unit_cost]"           value="{{ number_format($poi->unit_cost > 0 ? $poi->unit_cost : 1, 2, '.', '') }}">
                @else
                ...
                <div class="batch-row" data-poi="{{ $poiIdx }}" style="padding:14px 20px;border-bottom:1px solid var(--border);display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:12px;align-items:end">
                    ...
                    <div class="form-group" style="margin-bottom:0">
                        ...
                        <input type="number"
                               ...
                               placeholder="0"
                               oninput="recalcTotal()"
                               style="border:2px solid var(--primary);font-weight:700">
                    </div>
...
function toggleCompleted() {
    ...
    document.querySelectorAll('.item-completed').forEach(function(el) {
        el.style.display = completedVisible ? '' : 'none';
    });
    const btn = document.getElementById('toggle-completed-btn');
    if (btn) {
        btn.innerHTML = completedVisible
