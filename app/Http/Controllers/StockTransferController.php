...
class StockTransferController extends Controller
{
    ...
    /**
     * Admin: show the edit form for a transfer (including dispatched/completed ones).
     */
    public function edit(StockTransfer $transfer)
    {
        ...
        // currently holds its post-transfer balance, so it can give up at most
        // its current holding PLUS the quantity already transferred out.
        $maxNewQty = [];
        foreach ($transfer->items as $line) {
            $maxNewQty[$line->id] = (int) $line->sourceItem?->quantity + (int) $line->quantity;
        }

        return view('transfers.edit', compact('transfer', 'warehouses', 'maxNewQty'));
    }

    /**
     * Admin: apply corrections to a transfer — including one that has already
     * been dispatched — and reconcile every downstream record atomically.
     *
     * Strategy per line (correction of an inventory movement):
     *   old_qty = dispatched quantity stored in StockTransferItem
     *   new_qty = corrected quantity submitted by admin
     *   delta   = new_qty − old_qty
     *
     *   source item.quantity  −= delta   (transferring more → less left at source)
     *   dest   item.quantity  += delta   (transferring more → more at destination)
     *
     * Guards (both enforced against LOCKED rows):
     *   delta > 0 → source must physically hold the extra units.
     *   delta < 0 → destination must still hold the units being returned
     *               (they were not consumed by a later requisition/transfer).
     *
     * Stock cards are reconciled across EVERY transfer_out / transfer_in entry
     * generated by this line (partial dispatches create several): the delta is
     * applied newest-first so earlier shipment history survives, quantities
     * never go negative, and running balances for both items are rebuilt via
     * StockCardEntry::recalculateBalancesForItem().
     *
     * The line's planned quantity is corrected alongside the dispatched amount
     * (new_requested = old_requested − old_qty + new_qty) so outstanding plans
     * are preserved and the transfer status recomputes cleanly.
     *
     * The transfer ID, number, warehouses and Subsidy lineage are never
     * recreated — everything is corrected in place and written to the audit log.
     */
    public function update(Request $request, StockTransfer $transfer)
    {
        ...
        try {
            DB::transaction(function () use ($request, $transfer) {
                $oldDate = $transfer->transfer_date?->toDateString();
                ...
                foreach ($request->items as $idx => $line) {
                    ...
                    if ($delta > 0 && $sourceItem->quantity < $delta) {
                        throw ValidationException::withMessages([
                            "items.{$idx}.quantity" =>
                                ...
                                .$sourceItem->quantity.' unit(s) available. No changes were saved.',
                        ]);
                    }

                    // Decreasing: the DESTINATION must still hold the units being
                    // returned — they were not consumed by later transactions.
                    if ($delta < 0 && $destItem->quantity < -$delta) {
                        throw ValidationException::withMessages([
                            "items.{$idx}.quantity" =>
                                "Cannot correct \"{$destItem->description}\" from {$oldQty} to {$newQty} units: "
                                ."the destination warehouse ({$transfer->toWarehouse?->name}) currently holds only "
                                .$destItem->quantity.' unit(s), so '.(-$delta).' unit(s) cannot be returned to the source. '
                                .'No changes were saved.',
                        ]);
                    }

                    // ── Adjust source and destination stock ─────────────────
                    $newSourceQty = $sourceItem->quantity - $delta;
                    $newDestQty   = $destItem->quantity + $delta;

                    $sourceItem->update(['quantity' => $newSourceQty]);

                    // Unit cost travels with the transferred stock (existing rule).
                    $destItem->update(['quantity' => $newDestQty, 'unit_cost' => $newCost]);

                    // Preserve the originating Subsidy lineage on the destination stock.
                    if ($sourceItem->source_subsidy_status) {
                        $destItem->applySubsidySnapshot(
                            $sourceItem->source_subsidy_id,
                            $sourceItem->source_subsidy_ris,
                    ...
                    // A partially-dispatched transfer has several transfer_out /
                    // transfer_in entries; the correction is spread across all of
                    // them newest-first so no entry ever goes negative and no
                    // shipment row is silently dropped or duplicated.
                    $outEntries = StockCardEntry::where('reference_type', 'transfer_out')
                        ->where('reference_id', $transfer->id)
                        ->where('item_id', $sourceItem->id)
                        ->orderByDesc('id')
                        ->get();

                    $inEntries = StockCardEntry::where('reference_type', 'transfer_in')
                        ->where('reference_id', $transfer->id)
                        ->where('item_id', $destItem->id)
                        ->orderByDesc('id')
                        ->get();

                    if ($outEntries->isEmpty() && $inEntries->isEmpty()) {
                        // Nothing was dispatched through the normal flow yet —
                        // recording a quantity here CREATES the movement.
                        if ($newQty > 0) {
                            StockCardEntry::create([
                                'item_id'            => $sourceItem->id,
                            ...
                            $affectedItemIds[$sourceItem->id] = true;
                            $affectedItemIds[$destItem->id]   = true;
                        }
                    } else {
                        if ($delta !== 0) {
                            $this->spreadDeltaAcrossEntries($outEntries, $delta, 'issue_qty');
                            $this->spreadDeltaAcrossEntries($inEntries, $delta, 'receipt_qty');
                        }

                        // The transferred stock carries the line's unit cost —
                        // refresh every destination-side entry so receipts and
                        // totals reflect the corrected value.
                        foreach ($inEntries as $inEntry) {
                            $inEntry->update([
                                'receipt_unit_cost'  => $newCost,
                                'receipt_total_cost' => $inEntry->receipt_qty * $newCost,
                            ]);
                        }
                    ...
                    if ($delta !== 0 || abs($newCost - $oldCost) > 0.004 || $newQtyRequested !== $oldQtyRequested) {
                        $auditLines[] = [
                            ...
                            'inventory_effect' => [
                                'source'     => [$transfer->fromWarehouse?->name => -$delta],
                                'destination'=> [$transfer->toWarehouse?->name => $delta],
                            ],
                        ];
                    }
                }

                // ── Rebuild running stock-card balances for touched items ──
                foreach (array_keys($affectedItemIds) as $itemId) {
                    StockCardEntry::recalculateBalancesForItem($itemId);
                }

                // Refresh pending/partial/completed from the corrected lines.
                $transfer->load('items');
                $transfer->updateTransferStatus();
                ...
                if (! empty($auditLines)) {
                    StockTransferAuditLog::create([
                        ...
                        'changed_fields'    => [
                            ...
                            'transfer_date'         => [
                                'old' => $oldDate,
                                'new' => $transfer->fresh()->transfer_date?->toDateString(),
                            ],
                            'source_subsidy'        => $transfer->sourceSubsidyReference(),
                            'corrected_lines'       => $auditLines,
                        ],
                    ]);
                }
            });
        ...
        return redirect()
            ...
            ->with('success', 'Transfer corrected and stock reconciled at both warehouses.');
    }

    /**
     * Spread a quantity correction across a line's stock-card entries without
     * letting any entry go negative.
     *
     * Increases are absorbed by the NEWEST entry (earlier shipment history
     * stays untouched). Decreases consume entries newest-first until the
     * correction is exhausted.
     *
     * @param  \Illuminate\Support\Collection<int, StockCardEntry>  $entries  Newest first.
     */
    private function spreadDeltaAcrossEntries($entries, int $delta, string $field): void
    {
        if ($delta === 0 || $entries->isEmpty()) {
            return;
        }

        if ($delta > 0) {
            $entries->first()->increment($field, $delta);
        ...
        foreach ($entries as $entry) {
            ...
            $available = (int) $entry->{$field};
            if ($available <= 0) {
                continue;
            }
            $take = min($available, $remaining);
            $entry->decrement($field, $take);
            $remaining -= $take;
        }
    }
